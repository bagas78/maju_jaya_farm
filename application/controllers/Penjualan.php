...
class penjualan extends CI_Controller{
	...
	function transaksi_add(){
		if ( $this->session->userdata('login') == 1) {
			...
			//barang
			$data['barang_data'] = $this->query_builder->view("SELECT * FROM t_barang WHERE barang_kategori IN(1)");
	...
	function transaksi_view($id){
		...
		if ( $this->session->userdata('login') == 1) {
			...
			//barang
			$data['barang_data'] = $this->query_builder->view("SELECT * FROM t_barang WHERE barang_kategori IN(1,2)");
